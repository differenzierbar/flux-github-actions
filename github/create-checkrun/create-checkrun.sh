...
# update checkrun
checkrun_update="{\"name\":\"$name\",\"status\":\"completed\",\"conclusion\":\"$conclusion\",\"completed_at\":\"$(date -u +"%Y-%m-%dT%H:%M:%S"Z)\",\"output\":{\"title\":\"$name\",\"summary\":$(echo "$summary" | jq -Rsa .),\"text\":$(echo "$text" | jq -Rsa .)}}"
