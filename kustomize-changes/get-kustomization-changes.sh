...
while IFS= read -r kustomization_file; do
    ...
    if [[ "${changes_map[$kustomization_file_path]+exists}" ]]; then
        ...
        kustomization_path_real="$(realpath --relative-to $kustomizations_root $kustomization_path)"
        >&2 echo "kustomization_path_real: $kustomization_path_real"
        if [[ -f "$kustomizations_root/$kustomization_path_real/kustomization.yaml" ]];then
            ...
            >&2 echo "$kustomizations_root/$kustomization_path_real/kustomization.yaml not found"
            >&2 echo "ls -la: $(ls -la $kustomizations_root/$kustomization_path_real)"
